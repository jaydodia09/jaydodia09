...
class FCCLIP(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.test_topk_per_image = 2 #test_topk_per_image
        ...
        print("test meta data:", test_metadata)
        ...
        if self.training:
            ...
            if self.test_text_classifier is None:
                ...
                if not os.path.exists("text_classifier.pkl"):
                    ...
                    # this is needed to avoid oom, which may happen when num of class is large
                    bs = 64
#                    self.test_class_names = self.test_class_names[:98]
                    for idx in tqdm(range(0, len(self.test_class_names), bs)):
                        tx_outputs = self.backbone.get_text_classifier(self.test_class_names[idx:idx+bs], self.device).detach()
                        print("text class names:", self.test_class_names[idx:idx+bs], tx_outputs)
                        text_classifier.append(tx_outputs)
                    text_classifier = torch.cat(text_classifier, dim=0)
                    # average across templates and normalization.
                    text_classifier /= text_classifier.norm(dim=-1, keepdim=True)
                    text_classifier = text_classifier.reshape(text_classifier.shape[0]//len(VILD_PROMPT), len(VILD_PROMPT), text_classifier.shape[-1]).mean(1)
                    ...
                    pickle.dump(text_classifier.detach().cpu(), open("text_classifier.pkl", "wb"))
                else:
                    self.test_text_classifier = pickle.load(open("text_classifier.pkl", "rb")).to(self.device)
    ...
    def from_config(cls, cfg):
        ...
        print("config for sem seg head", cfg)
    ...
    def forward(self, batched_inputs):
        ...
        print("computed features:", features.keys())
